...
var Transaction=function(self){
	this.exec=function(cb){
		self.command("BEGIN TRANSACTION",function(err){
			if (err) {self.log(err,self);return;}
			cb(function(next,err3,ins){
				self.command("commit",function(err2){if (err) self.log(err2,self);});
				next(err3,ins);
			});
		});
	}
}
SQLite3.prototype.transaction=function(){
